...
namespace vtx {
	...
	public:
		...
		constexpr explicit matrix(Args... args) noexcept {
			static_assert(sizeof...(Args) <= 3 * 3, "Too many constructor arguments!");
			static_assert(
		...
		static constexpr matrix identity() noexcept { return matrix{1, 0, 0, 0, 1, 0, 0, 0, 1}; }
		...
		constexpr size_t rows() const noexcept { return 3; }

		constexpr size_t cols() const noexcept { return 3; }
