...
namespace vtx {
	...
	public:
		...
		constexpr explicit matrix(Args... args) noexcept {
			static_assert(sizeof...(Args) <= 2 * 2, "Too many constructor arguments!");
			static_assert(
			    all_convertible<Args...>::value, "All arguments must be convertible to T");
		...
		constexpr matrix transpose() const noexcept {
			return matrix(elements[0][0], elements[1][0], elements[0][1], elements[1][1]);
		}

		// Identity matrix (only for square matrices)
		static constexpr matrix identity() noexcept { return matrix(1, 0, 0, 1); }
		...
		constexpr matrix inverse() const noexcept {
			...
			return matrix<T, 2, 2>(elements[1][1] / det,
			    -elements[0][1] / det,
			    -elements[1][0] / det,
			    elements[0][0] / det);
		}
		...
		constexpr T trace() const noexcept { return elements[0][0] + elements[1][1]; }
		...
		constexpr size_t rows() const noexcept { return 2; }

		constexpr size_t cols() const noexcept { return 2; }
